...
fn stats() -> Result<Vec<String>, anyhow::Error> {
    ...
    results.push(format!(
        "Pending: {} TODO item(s), completed: {} TODO item(s).",
...
pub fn execute(cmd: Command) -> Result<Vec<String>, anyhow::Error> {
    match cmd {
        Command::Add(ContentCommand { todo }) => add(todo),
        Command::Remove(IndexCommand { index }) => remove(index),
        Command::Complete(IndexCommand { index }) => complete(index),
        Command::List => display(TodoType::Pending),
        Command::Audit => display(TodoType::All),
        Command::Stats => stats(),
        Command::Reset => reset(),
    }
}
